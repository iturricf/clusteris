...
class Presenter(object):
    ...
    def Process(self):
        ...
        processor = KMeans({'n_clusters': self.centroidsNumber})
        ...
        plotter = Plotter2D()
        plotter.PlotSamples(Dataset[:, 0], Dataset[:, 1], size=7, color=labels)

        if (len(centroids)):
            plotter.PlotCentroids(centroids[:, 0], centroids[:, 1], color='#050505', size=100)
